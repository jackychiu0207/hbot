        ...
        if data["dbfId"]==int(dict(interaction.data)['custom_id']):
            if "audio" in data:
                for i,name in enumerate(data["audio"]):
                    ...
                    elif "ERROR_JUST_PLAYED" in name.upper():options.append(SelectOption(label="手下還不能攻擊",description=name,value=str(i)+","+name))
                    ...
                    elif "TIMER" in name.upper():options.append(SelectOption(label="時間快不夠了",description=name,value=str(i)+","+name))
